...
class xKucoin:
    ...
    async def increase(self, increment: int, molecule: int):
        json_data = {'increment': increment, 'molecule': molecule}
        resp = await self.session.post('https://www.kucoin.com/_api/xkucoin/platform-telebot/game/gold/increase?lang=en_US', data=json_data)

        return '"success":true,"code":"200","msg":"success","retry":false,"data":' in await resp.text()
